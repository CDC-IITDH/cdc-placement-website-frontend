...
const AddStudent = ({
	...
	let initalResume = application_details
		? application_details.application_info.resume.name
		: '';
	let initialAdditionalInfo = application_details
		? JSON.parse(application_details.application_info.additional_info)
		: {};
	let initialEditoradd = id ? 'Edit' : '';

	const [student_id, setStudentId] = useState('');
	const [getApplicationResponse, setGetApplicationResponse] =
		useState(application_details);
	...
	useEffect(() => {
		initalResume = application_details
			? application_details.application_info.resume.name
			: '';
		initialAdditionalInfo = application_details
			? JSON.parse(application_details.application_info.additional_info)
			: {};
			...
			.then((res) => {
				...
				if (res.application_found === 'true') {
					updateAdditionalTextInfo(
						JSON.parse(res.application_info.additional_info)
					);
				}
				if (res.application_found === 'true') {
					setEditoradd('Edit');
	...
	const submitEdit = () => {
		setLoading(true);
		let app_id = getApplicationResponse.application_info
			? getApplicationResponse.application_info.id
			: '';
		if (id) {
		...
		<div>
			<Modal centered show={show} onHide={handleClose}>
					...
					<>
						<Modal.Body style={{ backgroundColor: '#dbdbdb' }}>
							...
							>
								...
								{reqJobPosting &&
									searchedId &&
									reqJobPosting?.additional_info.length !== 0 && (
										<>
												...
												<div className='modal-field-parent'>
													<Container>
														<Row>
															...
															<Col xs={12} sm={6}>
																<div className='modale-field-value'>
																	{getApplicationResponse.student_details.name}
																</div>
															</Col>
														</Row>
													</Container>
												...
												<div className='modal-field-parent'>
													<Container>
														<Row>
															...
															<Col xs={12} sm={6}>
																<div className='modale-field-value'>
																	{getApplicationResponse.student_details.batch}
																</div>
															</Col>
														</Row>
													</Container>
													...
													<Container>
														<Row>
															...
															<Col xs={12} sm={6}>
																<div className='modale-field-value'>
																	{
																		getApplicationResponse.student_details
																			.branch
																	}
																</div>
															</Col>
								...
								{reqJobPosting &&
								searchedId &&
								getApplicationResponse.student_details.resume_list ? (
									<div className='Parent'>
										...
										>
											{getApplicationResponse.student_details.resume_list.map(
												(resume, i) => {
													return (
				...
				<Modal.Footer style={{ backgroundColor: '#dbdbdb' }}>
					{!searchedId && !loading && (
						<Button onClick={idSubmit} className='btn-submit'>
							ADD STUDENT
						</Button>
					)}
					{!loading && searchedId && (
						<Button onClick={submitEdit} className='btn-submit'>
