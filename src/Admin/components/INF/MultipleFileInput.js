...
const MultipleFileInput = ({ stateVar, setStateVar, name, label }) => {
  ...
  function addFile(files, stateVar, setStateVar) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (files[i].type === "application/pdf" && files[i].size < 12485760) {
  ...
  return (
    <>
      <Form.Group className="mb-5">
        ...
        <div
          className="mt-2 bg-white rounded upload-zone text-center p-2 border border-secondary position-relative"
          style={{ color: "#b3bccd" }}
        >
          <svg
          ...
          <p className="text-dark">
            ...
            <span className="text-muted">
              Click here to select or drag and drop files here
            </span>
          </p>
        ...
        <Form.Text className="text-muted">PDF (Max. 10MB)</Form.Text>
      </Form.Group>
      <Dialog
        open={fileExceptions.length > 0 || sizeExceptions.length > 0}
        onClose={() => {
          setFileExceptions([]);
          setSizeExceptions([]);
        }}
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {fileExceptions.length > 0
              ? `${fileExceptions.join(", ")} ${
                  fileExceptions.length > 1 ? "are" : "is"
                } not ${fileExceptions.length > 1 ? "" : "a"} PDF file.`
              : ""}
            {sizeExceptions.length > 0
              ? `${sizeExceptions.join(", ")} ${
                  fileExceptions.length > 1 ? "are" : "is"
                } too large.`
              : ""}
      ...
      <Dialog
        open={filenumberExceptions}
        onClose={() => {
          setFilenumberExceptions(false);
        }}
      >
        <DialogTitle>File Exceptions</DialogTitle>
        <DialogContent>
