...
const Description = ({
	...
	useEffect(() => {
		if (
			...
			let desc = dashboardInfo[0][infoType].filter((elem) => {
				return elem.id.toString() === match.params.id.toString();
			});
			if (desc.length === 0) {
				desc = dashboardInfo[0]['placementApplication'].filter((elem) => {
					return elem.id.toString() === match.params.id.toString();
				});
			...
			if (desc.length !== 0) {
				...
				if (infoType === 'internships') {
					for (
						...
						if (
							dashboardInfo[0].internshipApplication[
								i
							].internship.toString() === match.params.id.toString() &&
							selectionStatus.resume_selected === null
						) {
							setHasApplied(true);
							setSelectionStatus({
								...selectionStatus,
					...
					for (
						...
						if (
							(dashboardInfo[0].placementApplication[i].id.toString() ===
								...
								dashboardInfo[0].placementApplication[i][
									'placement'
								].id.toString() === match.params.id.toString()) &&
							selectionStatus.resume_selected === null
						) {
							setHasApplied(true);
							setSelectionStatus({
					...
					return (
						<JobCard
							...
							selection_procedure_details_pdf_links={
								elem.selection_procedure_details_pdf_links
							}
							is_selection_procedure_details_pdf={
								elem.is_selection_procedure_details_pdf
							}
